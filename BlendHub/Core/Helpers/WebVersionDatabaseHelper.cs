    ...
    public static class WebVersionDatabaseHelper
    {
        ...
        public static async Task RefreshDatabaseAsync()
        {
            try
            {
                ...
                // Scan only current version and last 2 LTS versions
                var versionsToScan = new List<(string Name, string Url, string Date, long SizeBytes)>();
                
                var currentVersion = versionDirs.FirstOrDefault();
                if (currentVersion.Name != null)
                {
                    versionsToScan.Add(currentVersion);
                }

                var ltsVersions = versionDirs
                    .Where(e => {
                        var versionStr = e.Name.StartsWith("Blender", StringComparison.OrdinalIgnoreCase)
                            ? e.Name.Substring(7)
                            : e.Name;
                        var shortVersion = VersionHelper.GetShortVersion(versionStr);
                        // LTS versions: 5.2, 4.5, 3.6, 3.3, 2.93, 2.83
                        return shortVersion == "4.5" || shortVersion == "3.6" || shortVersion == "3.3" || shortVersion == "2.93" || shortVersion == "2.83" || shortVersion == "5.2";
                    })
                    .Take(2)
                    .ToList();

                foreach (var lts in ltsVersions)
                {
                    if (!versionsToScan.Any(v => v.Name == lts.Name))
                    {
                        versionsToScan.Add(lts);
                    }
                }
