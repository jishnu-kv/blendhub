...
namespace BlendHub.Services
{
    public class BlenderVersionInfo
    {
        ...
        public string FullVersion
        {
            get
            {
                if (string.IsNullOrEmpty(_fullVersion) && !string.IsNullOrEmpty(ExecutablePath) && System.IO.File.Exists(ExecutablePath))
                {
                    try
                    {
                        var vi = System.Diagnostics.FileVersionInfo.GetVersionInfo(ExecutablePath);
                        _fullVersion = vi.ProductVersion ?? vi.FileVersion ?? Version;
                    }
                    catch
                    {
    ...
    public class BlenderSettingsService
    {
        ...
        public Task<List<BlenderVersionInfo>> GetInstalledVersionsAsync(bool includeHidden = false)
        {
            return Task.Run(() => GetInstalledVersions(includeHidden));
        }
        ...
        public List<BlenderVersionInfo> GetInstalledVersions(bool includeHidden = false)
        {
            ...
            // Pre-fetch FullVersion on the background thread to cache ProductVersion info
