...
namespace BlendHub.Helpers
{
    public static class VersionHelper
    {
        ...
        public static bool IsNewerVersion(string webVersionStr, string installedVersionStr)
        {
            var webVer = ParseVersion(webVersionStr);
            var instVer = ParseVersion(installedVersionStr);

            int webMajor = webVer.Major;
            int webMinor = webVer.Minor;
            int webBuild = webVer.Build >= 0 ? webVer.Build : 0;
            int webRev = webVer.Revision >= 0 ? webVer.Revision : 0;

            int instMajor = instVer.Major;
            int instMinor = instVer.Minor;
            int instBuild = instVer.Build >= 0 ? instVer.Build : 0;
            int instRev = instVer.Revision >= 0 ? instVer.Revision : 0;

            if (webMajor != instMajor) return webMajor > instMajor;
            if (webMinor != instMinor) return webMinor > instMinor;
            if (webBuild != instBuild) return webBuild > instBuild;
            return webRev > instRev;
        }
    }
}
